...
- A `.NET Tasks` activity bar container hosts a tree view (`dotnetTasksKit.tasksView`) listing all `dotnet publish`/`dotnet build` tasks found in the workspace's tasks.json files (detected heuristically: `command` is `dotnet` and `args[0]` is `publish`/`build` — there is no marker property). Each item has inline **run**, **edit**, and **reveal output folder** (`revealFileInOS` on the task's `--output` resolved against `options.cwd`) buttons; tasks without a resolvable output get a `dotnetTaskNoOutput` context value that hides the reveal button. The view title bar hosts **create publish/build task** and **refresh** buttons; the view refreshes automatically via a `**/.vscode/tasks.json` file watcher.
